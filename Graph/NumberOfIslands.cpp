...
int isSafe(vector<int> M[], int row, int col, vector<vector<bool>> &visited,int ROW, int COL)
{
   
    return (row >= 0) && (row < ROW) && (col >= 0) && (col < COL) && (M[row][col] && !visited[row][col]);
}

void dfs(vector<int> A[], int i, int j, vector<vector<bool>> &vis, int N,int M) 
{
    ...
    {
        if ((i + row[k] < N && j + col[k] < M) &&(isSafe(A, i + row[k], j + col[k], vis, N,M))) { // checking if node is not visited
            dfs(A, i + row[k], j + col[k], vis, N, M); // and A[i+row[k]][j+col[k]] is 1, call dfs again
                    // exit(0);
        }
    }
}
